...
import {createProduct} from "@/lib/createProductApi";
import {useSiteContent} from "@/app/components/SiteContentProvider/SiteContentProvider";
import {appendUniqueImages, hasCreateProductFormDraft} from "@/lib/productFormState";
...
export const useCreateProductForm = (onProductCreated: () => void, onClose: () => void, pricingConfig?: PricingConfig | null) => {
    ...
    const copy = useSiteContent().createProduct;
    ...
    const handleBackdropClose = () => {
        ...
        if (hasCreateProductFormDraft(values)) {
            setError(null);
            onClose();
            return;
        }
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            await createProduct(values, copy.errors, values.image.length > 0 ? setUploadProgress : undefined);
            resetForm();
            onProductCreated();
            onClose();
        } catch (submitError) {
            if (submitError instanceof Error) {
                ...
                setError(copy.errors.unableToCreate);
            }
        } finally {
