...
export const CREATE_PRODUCT_MODAL_CLASS_NAMES = {
    ...
    panel: "vitan-sheet-panel relative flex max-h-[94vh] w-full max-w-3xl flex-col overflow-hidden rounded-[var(--radius-2xl)]",
    grabber: "hidden",
    ...
    checkbox: "h-5 w-5 rounded border-[var(--separator)] accent-[var(--label)]",
    hint: "text-xs font-normal text-[var(--text-tertiary)]",
    selectedImages: "grid gap-1 text-xs font-normal text-[var(--text-tertiary)]",
    existingImages: "flex min-h-24 gap-2 overflow-x-auto rounded-[var(--radius-sm)] bg-[var(--fill-tertiary)] p-2",
    ...
    selectedImagePreview: "block h-full w-full bg-cover bg-center",
    existingImageRemoveButton: "absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-[var(--destructive)] text-white shadow-lg shadow-black/15 disabled:cursor-not-allowed disabled:opacity-50",
    ...
    newImageBadge: "absolute bottom-1 left-1 rounded-full bg-black/65 px-2 py-0.5 text-[10px] font-semibold leading-none text-white",
    uploadProgress: "grid gap-2 rounded-[var(--radius-sm)] bg-[var(--fill-tertiary)] px-3 py-2 text-xs font-medium text-[var(--text-secondary)]",
    uploadProgressTrack: "h-1.5 overflow-hidden rounded-full bg-black/10",
    uploadProgressFill: "block h-full rounded-full bg-[var(--accent)] transition-[width] duration-200 ease-out",
    priceInfo: "grid gap-2 rounded-[var(--radius-sm)] bg-[var(--fill-tertiary)] px-4 py-3",
    priceInfoTitle: "text-[13px] font-semibold leading-[18px] text-[var(--text-secondary)]",
